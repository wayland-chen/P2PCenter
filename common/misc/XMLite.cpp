...
static const TCHAR szXMLPIOpen[] = ("<?");
static const TCHAR szXMLPIClose[] = ("?>");
static const TCHAR szXMLCommentOpen[] = ("<!--");
static const TCHAR szXMLCommentClose[] = ("-->");
static const TCHAR szXMLCDATAOpen[] = ("<![CDATA[");
static const TCHAR szXMLCDATAClose[] = ("]]>");
		...
		if( xml = _tcsskip( xml ) )
		{
			...
			if( pEnd == NULL ) 
			{
				...
				if( pi->erorr_occur == false ) 
				{
					...
					pi->error_string.Format("<%s> attribute has error ", (const char*)name);
				}
				return NULL;
			}
			...
			if( XIsEmptyString( value ) )
			{
				...
				if( pEnd == NULL ) 
				{
					if( pi->erorr_occur == false ) 
					{
						...
						pi->error_string.Format("%s must be closed with </%s>",(const char*)  name );
					}
					...
					if( xml = _tcsskip( xml ) )
					{
						...
						if( pEnd == NULL ) 
						{
							if( pi->erorr_occur == false ) 
							{
								...
								pi->error_pointer = xml;
								pi->error_code = PIE_NOT_CLOSED;
								pi->error_string.Format("it must be closed with </%s>",(const char*)name );
							}
							// error
							return NULL;
						}
						...
						else
						{
							...
							if( pi->force_parse == false )
							{
								...
								if( pi->erorr_occur == false ) 
								{
									...
									pi->error_string.Format("'<%s> ... </%s>' is not wel-formed.", (const char*) name, (const char*) closename );
								}
								return NULL;
							}
						}
